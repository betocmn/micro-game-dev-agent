...
async function getSnapshot(page: Page): Promise<GameSnapshot | null> {
  try {
    return await page.evaluate(() => {
      return (window as unknown as { __gameEval: { snapshot: () => GameSnapshot } }).__gameEval?.snapshot();
    });
  } catch {
    return null;
  }
...
export async function runInteractionEval(html: string): Promise<InteractionEvalResult> {
  ...
  try {
    ...
    try {
      await page.waitForFunction(
        () => (window as unknown as { __gameEval: { ready: boolean } }).__gameEval?.ready === true,
        { timeout: 5000 }
      );
    } catch {
      return {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      for (let j = 0; j < 5; j++) {
        const burstKey = ARROW_KEYS[Math.floor(Math.random() * ARROW_KEYS.length)];
  ...
  return { pass, durationMs, framesObserved, stateChanged, scoreChanged, crashed };
}

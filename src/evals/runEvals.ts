...
export async function runAllEvals(
  ...
  if (!runtime.pass) {
    ...
    return { runtime, interaction: defaultInteraction, judge: defaultJudge, summaryScore: 0 };
  }
  ...
  const judgeAvg =
    (judge.genreMatch + judge.mechanicMatch + judge.goalMatch + judge.controlsMatch + judge.coherence) / 5;
